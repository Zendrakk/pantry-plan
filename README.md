...
- **Resilient database connections and centralized error handling.** EF Core is configured to automatically retry transient database failures (e.g. Neon's serverless instance waking from scale-to-zero) rather than failing the request outright. A global exception handler catches any unhandled error, logs it with full context, and returns a generic error response — so an unexpected failure never leaks internal details (stack traces, exception messages) to the client.

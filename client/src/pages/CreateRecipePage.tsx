...
import type { IngredientLine, Unit } from '../types/recipe'

// The full list of valid units, matching the backend's Unit enum.
// We use this to build the dropdown options for each ingredient row.
...
// A single blank ingredient row, used both for the initial state and
// whenever the user clicks "Add Ingredient."
...
function CreateRecipePage() {
  ...
  const [ingredients, setIngredients] = useState<IngredientLine[]>([createBlankIngredientLine()])
  ...
  // Updates one field of one specific ingredient row, identified by its
  // position in the array, without mutating the original array.
  function handleIngredientFieldChange(
    ...
    const updatedIngredients = ingredients.map(function (ingredient, currentIndex) {
      if (currentIndex !== index) {
        // Not the row we're editing - return it completely unchanged.
        return ingredient
      }

      // This is the row we're editing - return a new object with just
      // the one field replaced.
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const createdRecipe = await createRecipe(auth.accessToken, {
        title: title,
      ...
      if (error instanceof ApiError) {
        setErrorMessage('Failed to create recipe. Please check your entries.')
      } else {
